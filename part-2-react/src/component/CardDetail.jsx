const CardDetail = ({ list, title, subTitle, price, subTitle2, icon,recomment }) => {
  //
  return (
    <>
      <div className={` ${recomment===true? "bg-blue-100":''} border  inline-block border-gray-300 rounded-2xl p-5`}>
        <div className="header" style={{height:"10rem"}}>
          <div className="flex gap-2 justify-start items-center mb-3">
            <div className="icon">{icon}</div>
            <div className="w-full">
              <div className=" flex items-center justify-between">
                <p className="font-bold">{title}</p>
                <span className={` ${recomment===true? "":'hidden'} bg-blue-700 text-white px-2 py-1 font-medium rounded-xl`} style={{fontSize:"0.75rem"}}>Đề xuất</span>
              </div>
              <span className="font-medium" style={{ color: "#5F5F67" }}>
                {subTitle}
              </span>
            </div>
          </div>
          <div className="font-medium" style={{ color: "#3F3F45" }}>
            {subTitle2}
          </div>
        </div>
        <div className="main">
          <div className="mb-7">
            <p className="font-medium" style={{ color: "#7D7D85" }}>
              Chỉ từ
            </p>
            <span className="font-bold text-2xl">{price}</span>{" "}
            <span className="font-medium" style={{ color: "#7D7D85" }}>
              /tháng
            </span>
          </div>
          <div className="flex flex-1 mb-5">
            <button className="btn font-bold hover:bg-gray-700 transition-all cursor-pointer bg-black text-white py-3 rounded-xl flex flex-1 items-center justify-center">
              Tư vấn dùng thử
            </button>
          </div>
          <div className="">
